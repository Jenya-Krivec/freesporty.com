...
class ProsCons {
    ...
    createRow(table, column, id, lang) {
        ...
        const classes = column === 'pros' ? 'text-violet-500 bg-violet-100' : 'text-gray-500 bg-gray-100';
        const child = column === 'pros' ? 0 : 1;
    ...
    add(id){
        const row = '' +
            ...
            '    <div class="flex items-center text-white bg-violet-500 rounded-full cursor-pointer transition-all duration-500 absolute -bottom-3 -left-4 add-after hidden" onclick="new AddSection().addComponent({target: this})" data-id="'+(id+1)+'" title="Add component after Pros &amp; Cons">\n' +
            ...
            '                <div class="flex-row flex justify-start items-center text-violet-500 bg-violet-100 w-full h-6 rounded-sm mr-2 px-2">Pros</div>\n' +
            '            </div>\n' +
            '            <div class="flex-row flex justify-start items-center py-2 w-full">\n' +
            '                <div class="flex-row flex justify-center items-center text-violet-500 bg-violet-100 w-6 h-6 rounded-sm mr-2 flex-shrink-0">✓</div>\n' +
            ...
            '            </div>\n' +
                        '<div class="flex items-center justify-center self-center text-white bg-violet-500 rounded-full h-6 w-6 transition-all duration-500 mb-1 cursor-pointer add-pros" onclick="new ProsCons().addRow(`pros`, {target: this})" data-id="'+(id+1)+'" title="Add Pros">+</div>\n' +
            '        </div>\n' +
            '        <div class="w-full sm:w-1/2 flex-col justify-center items-start flex cons">\n' +
            '            <div class="flex-row flex justify-start items-center py-2 w-full">\n' +
            '                <div class="flex-row flex justify-start items-center text-gray-500 bg-gray-100 w-full h-6 rounded-sm mr-2 px-2">Cons</div>\n' +
            '            </div>\n' +
            '            <div class="flex-row flex justify-start items-center py-2 w-full">\n' +
            '                <div class="flex-row flex justify-center items-center text-gray-500 bg-gray-100 w-6 h-6 rounded-sm mr-2 flex-shrink-0">X</div>\n' +
            ...
            '            </div>\n' +
            '            <div class="flex items-center justify-center self-center text-white bg-violet-500 rounded-full h-6 w-6 transition-all duration-500 mb-1 cursor-pointer add-cons" onclick="new ProsCons().addRow(`cons`, {target: this})" data-id="'+(id+1)+'" title="Add Cons">+</div>\n' +
            '        </div>\n' +
            '    </div>\n' +
            ...
            '        </div>\n' +
            '    </div>\n' +
            '    <select class="absolute top-1 -right-11 border-2 border-violet-500 text-xs rounded language" data-id="'+(id+1)+'" onChange="new Language().toggle({target: this})" title="Change language for Pros &amp; Cons">\n' +

...
@section('content')
    <main class="pt-24">
        ...
        <form action="{{ route('admin.storeCurrencies') }}" method="POST" enctype="multipart/form-data" class="flex justify-center items-center flex-col form">
            ...
            <span class="flex items-center justify-center self-center text-white bg-violet-500 rounded-full h-6 w-6 transition-all duration-500 mb-2 mx-auto cursor-pointer add-currency" data-id="{{$currency['id']}}" title="Add Currency">+</span>
        </form>
    </main>

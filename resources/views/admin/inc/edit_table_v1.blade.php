<div class="relative border-b-2 border-gray-200">
    <div class="flex items-center text-white bg-violet-500 rounded-full cursor-pointer transition-all duration-500 absolute -bottom-3 -left-4 hidden add-after" data-id="{{$id}}" title="Add component after Pros & Cons">
    ...
    <div data-name="{{$id.'-'.$lang}}" class="flex flex-col sm:flex-row justify-center items-start @if($lang !== 'en') hidden @endif">
        <div class="w-full sm:w-1/2 flex-col justify-center items-start flex mr-2 pros">
            <div class="flex-row flex justify-start items-center py-2 w-full">
                <div class="flex-row flex justify-start items-center text-violet-500 bg-violet-100 w-full h-6 rounded-sm mr-2 px-2">@lang('review.Pros')</div>
            </div>
            @foreach(json_decode($value, true)[0] as $i => $pros)
                <div class="flex-row flex justify-start items-center py-2 w-full">
                    <div class="flex-row flex justify-center items-center text-violet-500 bg-violet-100 w-6 h-6 rounded-sm mr-2 flex-shrink-0">&check;</div>
            ...
            @endif
            <div class="flex items-center justify-center self-center text-white bg-violet-500 rounded-full h-6 w-6 transition-all duration-500 mb-1 cursor-pointer add-pros" data-id="{{$id}}" title="Add Pros">+</div>
        </div>
        <div class="w-full sm:w-1/2 flex-col justify-center items-start flex cons">
            ...
            @endif
            <div class="flex items-center justify-center self-center text-white bg-violet-500 rounded-full h-6 w-6 transition-all duration-500 mb-1 cursor-pointer add-cons" data-id="{{$id}}" title="Add Cons">+</div>
        </div>
    </div>
    @endforeach
    <select class="absolute top-1 -right-11 border-2 border-violet-500 text-xs rounded language" data-id="{{$id}}" title="Change language for Pros & Cons">
